...
interface TwikooRecentResponse {
	data?: RecentComment[]
	accessToken?: string
}
...
async function fetchRecentComments() {
	...
	if (!envId)
		throw new Error('Twikoo 未配置 envId')
	...
	<template v-else>
		<ZError v-if="error">
			...
			<template #description>
				{{ error?.message || '无法获取最新评论' }}
			</template>
		</ZError>

		<div v-else-if="!list.length" class="rc-empty">
			暂无评论
		</div>
